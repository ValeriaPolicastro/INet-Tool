...
We load the package and a list with the example datasets. In this case, columns of both datasets represent patients, while rows represent gene expression, methylation and mirna expression respectively.
...
The outputs of the function are an histogram of the weights for each dataset and a list of lists containing:
...
To construct the *Consensus Network* use the list of adjacency matrices:
...
adjL <- net$Adj
Con <- consensusNet (adjL=net$Adj, theta=0.05) 
